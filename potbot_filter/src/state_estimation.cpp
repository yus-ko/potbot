...
FilterClass::FilterClass()
{
	...
	sub_obstacle_scan_				= nhSub_.subscribe("obstacle/scan/clustering",			1,&FilterClass::__obstacle_scan_callback,this);
    sub_obstacle_pcl_				= nhSub_.subscribe("obstacle/pcl/clustering",			1,&FilterClass::__obstacle_pcl_callback,this);
...
void FilterClass::__obstacle_scan_callback(const potbot_msgs::ObstacleArray& msg)
{
    ...
    for(auto& obstacle : obstacle_array.data)
    {
        ...
        if (iter != ukf_id.end())
        {
            ...
            observed_data<< x, y;
            std::tuple<Eigen::VectorXd, Eigen::MatrixXd, Eigen::MatrixXd> ans = states_ukf_scan_[index_ukf].update(observed_data,dt);
        ...
        else
        {
            ...
            states_ukf_scan_.push_back(estimate);
            ukf_id.push_back(id);
        }
    ...
    //     for (auto& state : states_ukf_scan_)
    //     {
...
void FilterClass::__obstacle_pcl_callback(const potbot_msgs::ObstacleArray& msg)
{
    potbot_msgs::ObstacleArray obstacle_array = msg;
    static std::vector<int> ukf_id;
    ...
    for(auto& obstacle : obstacle_array.data)
    {
        ...
        if (iter != ukf_id.end())
        {
            ...
            Eigen::VectorXd observed_data(ny);
            observed_data<< x, y;
            std::tuple<Eigen::VectorXd, Eigen::MatrixXd, Eigen::MatrixXd> ans = states_ukf_pcl_[index_ukf].update(observed_data,dt);
            Eigen::MatrixXd xhat = std::get<0>(ans);
            Eigen::MatrixXd P = std::get<1>(ans);
            Eigen::MatrixXd K = std::get<2>(ans);
        ...
        else
        {
            ...
            states_ukf_pcl_.push_back(estimate);
            ukf_id.push_back(id);
        }
    ...
    pub_obstacles_pcl_.publish(obstacle_array);
