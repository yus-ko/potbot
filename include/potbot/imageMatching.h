...
//self msg
#include <potbot/MaskImageData.h>
#include <potbot/ImageMatchingData.h>
// rqt_reconfige
#include <dynamic_reconfigure/server.h>
//#include <potbot/imageMatchingConfig.h>
//debug
...
class imageMatchingClass{
    private:
        ...
        potbot::MaskImageData midPre,midCur;
        ...
        // dynamic_reconfigure::Server<potbot::imageMatchingConfig> server;
        // dynamic_reconfigure::Server<potbot::imageMatchingConfig>::CallbackType f;
    ...
    public:
        ...
        //セット：内部パラメータの書き込み
        void setLaunchParam();
        //void configCallback(potbot::imageMatchingConfig &config, uint32_t level);
        //ゲット：内部パラメータの読み込み
        int& getParam();//(未使用)
        //
        //in methods.cpp
        ...
        void maskImage_callback(const potbot::MaskImageData::ConstPtr& msg);//データ受信(mask画像)
        //--グレースケール化
        void cvtGray();
        //--データ確認
